...
import tr.com.huseyinaydin.controller.TutorialController;
import tr.com.huseyinaydin.model.Tutorial;
import tr.com.huseyinaydin.repository.TutorialRepository;
import com.fasterxml.jackson.databind.ObjectMapper;

@WebMvcTest(TutorialController.class) // 🧠 Sadece TutorialController’ı yükler; servis ve veri katmanlarını devre dışı bırakır.
public class TutorialControllerTests {

    @MockBean
	private TutorialRepository tutorialRepository;  
	// 🎭 Gerçek veri tabanını kullanmadan Repository davranışını taklit eder; testin izole biçimde çalışmasını sağlar.

	@Autowired
	private MockMvc mockMvc;  
	// 🧪 HTTP isteklerini ve yanıtlarını simüle eder; Controller katmanını gerçek bir web sunucusu olmadan test etmeyi mümkün kılar.

	@Autowired
	private ObjectMapper objectMapper;  
	// 🧩 Java nesnelerini JSON formatına dönüştürür ve JSON verilerini nesnelere parse eder; testlerde veri alışverişini kolaylaştırır.

    @Test
    void shouldCreateTutorial() throws Exception {
        Tutorial tutorial = new Tutorial(1, "Behlül", "Kartalkaya", true);
    ...
    void shouldReturnTutorial() throws Exception {
        long id = 1L;
        Tutorial tutorial = new Tutorial(id, "Erol", "Ahmet", true);
